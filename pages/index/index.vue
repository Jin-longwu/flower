		...
		<scroll-view scroll-y>
			<swiper :autoplay="true" :interval="2000" circular>
				<swiper-item v-for="item in message.goodsData.goods.pictures" :key="item.id">
					<image :src="item.url" mode="widthFix" @click="previewImg(item.url)"></image>
				</swiper-item>
			</swiper>
			<view class="price">
				<view class="flowerPrice">￥{{message.goodsData.skuInfo.spuPrice.price/100}}</view>
				<view class="text">
					<view class="mess">
			...
			<view class="flower">
				<view class="flowertop">
					<image :src="message.mpData.logo"></image>
					<view class="shopName">{{message.mpData.shopName}}</view>
					<button>进店逛逛</button>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getcontent() {
				let result1 = await myRequestGet("/wscshop/goods/showcase-components.json?alias=26xr56g4mjbq2&kdt_id=10056586")
